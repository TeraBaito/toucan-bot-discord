...
async function checkStaff(member) {
    ...
    try {
        if (
            member.roles.cache.find(r => r.name == 'Moderator') || // Check by roles
            member.roles.cache.find(r => r.name == 'Mega Moderater') || // I swear dude, the role has a typo
...
function blacklistProcess(message, bot) {
    const { nsfw, offensive } = readJSONSync('./src/handlers/blacklisted-words.json'),
    ...
    */
    ...
    if (nsfw.some(process)) {
        act('Highly NSFW tinted words are discouraged in this server');
    }

    // Offensive words
    if(offensive.some(process)) {
        act('Really offensive words are discouraged in this server');
    }
}
